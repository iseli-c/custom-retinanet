...
def collater(data):
    ...
    return {'img_fname':img_fnames,'img': padded_imgs, 'annot': annot_padded, 'scale': scales}

class Resizer(object):
    """Convert ndarrays in sample to Tensors."""

    def __call__(self, sample, min_side=1024, max_side=1024): #608
        image_fname,image, annots = sample['img_fname'],sample['img'], sample['annot']
        ...
        return {'img_fname':image_fname,'img': torch.from_numpy(new_image), 'annot': torch.from_numpy(annots), 'scale': scale}
...
class Augmenter(object):
    ...
    def __call__(self, sample, flip_x=0.5):
        ...
        if np.random.rand() < flip_x:
            ...
            sample = {'img_fname':image_fname,'img': image, 'annot': annots}
...
class Normalizer(object):
    ...
    def __call__(self, sample):
        ...
        return {'img_fname':image_fname,'img':((image.astype(np.float32)-self.mean)/self.std), 'annot': annots}
